...
# FIX: unpack 11 giá trị (reports_pkg = None placeholder)
...
with tab_multiples:
    ...
    # Hiển thị thông tin pha loãng nếu có
    dilution_years = valuation_pkg.get('dilution_years', [])
    if dilution_years:
        st.warning(
            f"⚠️ **Lưu ý pha loãng:** {ticker_input} đã phát hành thêm CP trong "
            f"năm **{', '.join(str(y) for y in dilution_years)}** "
            f"(cổ tức cổ phiếu hoặc phát hành quyền). "

...
def dcf_fcff_scenarios(
    ...
    for name, params in scenarios.items():
        ...
        try:
            ...
            if fair_price > 0:
                results[name] = round(fair_price, 0)
        except Exception:
            continue
...
def nine_methods_valuation(
    ...
    # 4–6. DCF kịch bản
    if dcf_results:
        for scenario_name, price in dcf_results.items():
            methods[f'DCF {scenario_name}'] = price
...
def summarize_valuation(methods: dict, current_price: float) -> dict:
    """
    Tính giá trung bình từ tất cả phương pháp (bỏ qua key nội bộ _xxx).
    Trả về dict với upside/downside so với giá thị trường.
    """
    ...
    upside_pct = (avg_fair / current_price - 1) * 100

    signal = "TIỀM NĂNG TĂNG"  if upside_pct >  15 else \
             "ĐỊNH GIÁ HỢP LÝ" if upside_pct > -10 else \
             "THẬN TRỌNG"

    return {
        'avg_fair_price':  round(avg_fair, 0),
        'median_fair_price': round(med_fair, 0),
        'upside_pct':      round(upside_pct, 1),
        'signal':          signal,
        'n_methods':       len(prices),
        'price_range_low': round(min(vals), 0),
        'price_range_high':round(max(vals), 0),

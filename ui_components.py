...
def render_tab_multiples(metrics, fundamentals, valuation_pkg):
    ...
    # ── FIX: Lấy đúng raw values để tính ratio ───────────────────────────
    # ebitda_latest_billion = EBITDA tuyệt đối (tỷ đồng) — KHÔNG phải ratio
    ebitda_b    = metrics.get("ebitda_latest_billion",  0.0) or 0.0
    # cfo_latest_billion = CFO tuyệt đối (tỷ đồng)
    cfo_b       = metrics.get("cfo_latest_billion",     0.0) or 0.0
    # revenue_latest_billion = Doanh thu tuyệt đối (tỷ đồng)
    rev_b       = metrics.get("revenue_latest_billion", 0.0) or 0.0
    # net_debt_billion = Nợ ròng (tỷ đồng), có thể âm nếu tiền > nợ
    net_debt_b  = metrics.get("net_debt_billion",       0.0) or 0.0

    # EV = Market Cap + Net Debt
    ev_b = mktcap_b + net_debt_b

    # Tính ratio — chỉ khi mẫu số > 0
    ...
    # Fallback: thử lấy từ fundamentals nếu metrics không có
    if ev_ebitda_x is None:
        ebitda_b_fb = fundamentals.get("ebitda_latest_billion", 0.0) or 0.0
        if ebitda_b_fb > 0 and ev_b > 0:
            ev_ebitda_x = ev_b / ebitda_b_fb

    if pcf_x is None:
        cfo_b_fb = fundamentals.get("cfo_latest_billion", 0.0) or 0.0
        if cfo_b_fb > 0 and mktcap_b > 0:
            pcf_x = mktcap_b / cfo_b_fb

    if ps_x is None:
        rev_b_fb = fundamentals.get("revenue_latest_billion", 0.0) or 0.0
        if rev_b_fb > 0 and mktcap_b > 0:
            ps_x = mktcap_b / rev_b_fb
...
Với ngân hàng nên dùng <strong>P/B + ROE, NIM, NPL, CAR</strong> thay thế.
        ...
        # Card thứ 4: Price/Book tính thêm để không để trống
        net_assets_b = metrics.get("equity_billion", 0.0) or 0.0
        price_to_assets_x = (mktcap_b / net_assets_b) if net_assets_b > 0 and mktcap_b > 0 else None
        pa_str   = f"{price_to_assets_x:.2f}x" if price_to_assets_x is not None else "—"
        pa_color = _val_color(price_to_assets_x, (1.0, 2.0)) if price_to_assets_x is not None else "na"
        pa_sub   = (
            "<strong>Dưới book value</strong>" if price_to_assets_x and price_to_assets_x < 1.0
            else "Hợp lý" if price_to_assets_x and price_to_assets_x < 2.0
            else "Premium cao" if price_to_assets_x
        ...
        # Chỉ hiện note nếu vẫn còn thiếu sau fallback
        missing = []
        if ev_ebitda_x is None: missing.append("EBITDA")
        if pcf_x       is None: missing.append("CFO")
        if ps_x        is None: missing.append("Doanh thu")
        if missing:
            st.markdown(f"""
<div class="mult-note">
ℹ️ Một số chỉ số chưa tính được do <strong>thiếu dữ liệu {' / '.join(missing)}</strong>

...
CHANGELOG:
  ...
  - FIX render_tab_valuation: lọc None trong methods, key summary đúng
  - FIX render_tab_dcf: tương thích dcf_scenarios={label: float}
  - FIX render_tab_dupont: dùng đúng tên cột từ dupont_decomposition()
  - FIX render_tab_forecast: summary dùng low_value/high_value/upside_pct
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# FIX: render_tab_valuation
#   - Lọc None khỏi methods trước khi vẽ → tránh TypeError
#   - Dùng đúng key: median_value, upside_pct (thay vì median, upside_median_pct)
# ══════════════════════════════════════════════════════════════════════════════
def render_tab_valuation(valuation_pkg, metrics):
    ...
    methods = valuation_pkg.get('methods', {}) or {}

    if not summary:
        st.warning("Không đủ dữ liệu để chạy các phương pháp định giá.")
        return

    # FIX: đọc đúng key từ summarize_valuation(), fallback key cũ
    median_val  = summary.get('median_value') or summary.get('median') or 0
    upside_pct  = summary.get('upside_pct')   or summary.get('upside_median_pct') or 0

    st.markdown(f"#### Giá Trị Hợp Lý Ước Tính: **{median_val:,.0f} đ/CP**")
    c1, c2 = st.columns([1, 2])
    c1.metric("Verdict", summary.get('verdict', '—'))
    ...
    if methods:
        ...
        # FIX: chỉ giữ lại giá trị hợp lệ (không None, > 0)
        valid_methods = {
            k: v for k, v in methods.items()
            if v is not None and isinstance(v, (int, float)) and v > 0
        }
        if valid_methods:
            cols = st.columns(min(len(valid_methods), 4) or 1)
            for i, (name, value) in enumerate(valid_methods.items()):
...
# ══════════════════════════════════════════════════════════════════════════════
# FIX: render_tab_dcf
#   - dcf_scenarios mới = {label: float}, cũ = {label: dict}
#   - Tìm key linh hoạt ("Bi quan" khớp với "DCF Bi quan")
# ══════════════════════════════════════════════════════════════════════════════
def render_tab_dcf(valuation_pkg, metrics):
    ...
    if dcf:
        ...
        def _find_dcf(dcf_dict, short_name):
            """Tìm key chứa short_name (không phân biệt hoa thường)."""
            for k, v in dcf_dict.items():
        ...
        for name, icon, tone in order:
            res = _find_dcf(dcf, name)
            if res is None:
                continue

            # FIX: tương thích cả 2 format
            if isinstance(res, dict):
                price_per_share = res.get('value_per_share', 0) or 0
                wacc_pct = (res.get('wacc', 0) or 0) * 100
                g_pct    = (res.get('g',    0) or 0) * 100
                sub_line = f"WACC {wacc_pct:.0f}% · g {g_pct:.1f}%"
            else:
                price_per_share = float(res)
                sub_line = ""

            if not price_per_share:
                continue
            pct = (price_per_share / current_price - 1) * 100 if current_price else 0
...
<div class="dcf-card dcf-card-{tone}">
  ...
  <div class="dcf-card-sub">{sub_line}</div>
  <div class="dcf-card-bottom"><span></span>
    <div style="text-align:right;">
      <div class="dcf-card-value val-{tone}">{_fmt_k(price_per_share)}</div>
...
# ══════════════════════════════════════════════════════════════════════════════
# FIX: render_tab_dupont
#   - Tên cột thực tế từ dupont_decomposition():
#     'Net Margin (%)' / 'Asset Turnover' / 'Equity Multiplier' / 'ROE (%)' / 'Năm'
#   - Auto-map linh hoạt, tương thích cả cột snake_case cũ
# ══════════════════════════════════════════════════════════════════════════════
def render_tab_dupont(df_dupont):
    ...
    # Auto-map tên cột
    nm_col = at_col = lev_col = None
    for c in df.columns:
        cl = c.lower()
        if 'net_margin' in cl or 'net margin' in cl or (cl.startswith('net') and 'margin' in cl):
            nm_col = c
        elif 'asset_turnover' in cl or 'asset turnover' in cl:
            at_col = c
        elif 'leverage' in cl or 'equity_multiplier' in cl or 'equity multiplier' in cl:
            lev_col = c

    def _get_display_vals(col):
        if col is None or col not in df.columns:
            return None
        s = df[col]
        # Nếu dạng thập phân (0.15 thay vì 15.0) thì nhân 100
        if s.dropna().abs().max() <= 2:
            s = s * 100
        return s

    fig = go.Figure()
    nm_vals  = _get_display_vals(nm_col)
    at_vals  = _get_display_vals(at_col)
    lev_vals = _get_display_vals(lev_col)

    if nm_vals is not None:
        fig.add_trace(go.Bar(x=df.index, y=nm_vals,  name='Biên LN (%)',  marker_color='#a855f7'))
    if at_vals is not None:
        fig.add_trace(go.Bar(x=df.index, y=at_vals,  name='Vòng quay TS', marker_color='#ec4899'))
    if lev_vals is not None:
        fig.add_trace(go.Bar(x=df.index, y=lev_vals, name='Đòn bẩy',      marker_color='#06b6d4'))
    ...
    if not df.empty:
        ...
        r = df.loc[latest_year]
        d1, d2, d3 = st.columns(3)

        def _fmt_raw(col, is_pct=False):
            if col is None or col not in r.index:
                return "—"
            v = float(r[col])
            if is_pct:
                display = v * 100 if abs(v) <= 2 else v
                return f"{display:.1f}%"
            else:
                display = v * 100 if abs(v) <= 2 else v
                return f"{display:.4f}x"

        d1.metric(f"Biên LN {latest_year}", _fmt_raw(nm_col,  is_pct=True))
        d2.metric("Vòng Quay TS",           _fmt_raw(at_col,  is_pct=False))
        d3.metric("Đòn Bẩy",               _fmt_raw(lev_col, is_pct=False))
...
# ══════════════════════════════════════════════════════════════════════════════
# FIX: render_tab_forecast
#   - summary dùng đúng key: low_value/high_value/upside_pct (thay p25/p75)
# ══════════════════════════════════════════════════════════════════════════════
def render_tab_forecast(df_5y_table, fundamentals, metrics, tech, valuation_pkg, period_col='Năm'):
    ...
    if summary:
        ...
        # FIX: dùng đúng key từ summarize_valuation(), fallback key cũ
        upside_median = summary.get('upside_pct') or summary.get('upside_median_pct') or 0
        low_val  = summary.get('low_value')  or summary.get('p25')
        high_val = summary.get('high_value') or summary.get('p75')
        ...
        target_str = (
            f"₫{low_val:,.0f} – {high_val:,.0f}"
            if low_val is not None and high_val is not None else "—"
        )
...
def render_tab_multiples(metrics, fundamentals, valuation_pkg):
    ...
    st.markdown(
        '<div class="mult-section-title">01 &nbsp; Định giá cốt lõi · P/E · P/B · EPS · BVPS</div>',
...
Với ngân hàng nên dùng <strong>P/B + ROE, NIM, NPL, CAR</strong> thay thế.
        ...
        cards_ext = (
          ...
          + _mult_card("Vốn hóa / Tài sản", yr, "—", "na", "Market Cap / Net Assets")
        )

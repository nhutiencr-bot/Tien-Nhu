...
def render_tab_kqkd(df_5y_table, fundamentals, period_col='Năm'):
    ...
    for col in indicator_cols:
        ...
        if is_pct:
            valid_v = [v for v in vals if pd.notnull(v)]
            if len(valid_v) >= 2:
            ...
            # per-share metrics: show simple % change first→last
            valid_v = [(i, v) for i, v in enumerate(vals) if pd.notnull(v) and v != 0]
            if len(valid_v) >= 2:

...
def execute_equity_research_pipeline(ticker):
    try:
        ...
        if source_used != 'VCI':
            pass  # source fallback logged internally
        ...
        try:
            ...
            pass  # quarterly table error (internal)

        df_dupont = dupont_decomposition(
            revenue_series, net_profit_series, total_assets_series, equity_series)

        # Dùng LẠI cfo_series_for_multiples đã mở rộng đầy đủ từ khoá ở mục 5b
        # (trước đây DCF dùng 1 biến cfo_series riêng, hẹp hơn — chỉ 3 từ khoá
        # — khiến FCFF ra rỗng dù CFO đã tìm được ở tab Multiples. Gộp lại
        # dùng chung 1 nguồn để nhất quán.)
        ...
        # DEBUG DCF removed (internal logging only)

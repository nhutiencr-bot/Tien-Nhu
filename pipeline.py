...
def _build_engines_with_fallback(ticker):
    ...
    raise ConnectionError(
        f"Không lấy được dữ liệu cho mã {ticker} từ bất kỳ nguồn nào. Lỗi: {last_error}"
    )
...
def execute_equity_research_pipeline(ticker):
    try:
        ...
        # ── SỬA LỖI TẬN GỐC: ĐIỀN DỮ LIỆU CAFEF TỪNG NĂM ĐỘC LẬP ──
        try:
            ...
            # BƯỚC 1: Thử lấy 4 chỉ tiêu cốt lõi từ API Đầy Đủ
            try:
                cafef_full = fetch_cafef_yearly_full(ticker, years=TARGET_YEARS)
                if isinstance(cafef_full, dict):
            ...
            # BƯỚC 2: Vớt vát Tài Sản & Vốn Chủ từ Bảng Cân Đối nếu Bước 1 hụt
            try:
                ...
                if missing_eq or missing_ta:
                    cafef_bs = fetch_cafef_balance_sheet_5y(ticker, end_year=LAST_DATA_YEAR)
                    if isinstance(cafef_bs, dict):
                        equity_series       = _fill_missing(equity_series,       cafef_bs.get('equity',       pd.Series(dtype=float)))
                        total_assets_series = _fill_missing(total_assets_series, cafef_bs.get('total_assets', pd.Series(dtype=float)))
            except Exception:
                pass
        ...
        cfo_series_for_multiples = normalize_to_billion_vnd(find_row_series(
            df_cashflow,
            ['lưu chuyển tiền thuần từ hoạt động kinh doanh', 'lưu chuyển tiền thuần từ hđkd',
             'i. lưu chuyển tiền từ hoạt động kinh doanh', 'net cash flow from operating', 
             'net cash provided by operating', 'net cash from operating activities',
        ...
        # ── BẢNG 5 NĂM (ĐÃ CHẶN LỖI HIỂN THỊ NaN BẰNG CHUỖI GẠCH NGANG) ──
        ...
        for y in years_available:
            ...
            if (y not in eps_series_filled.index or pd.isna(eps_series_filled.get(y))) and has_np and issue_share > 0:
                eps_series_filled[y] = net_profit_series[y] * 1e9 / issue_share
            if (y not in bvps_series_filled.index or pd.isna(bvps_series_filled.get(y))) and has_eq and issue_share > 0:
                bvps_series_filled[y] = equity_series[y] * 1e9 / issue_share
            if (y not in roe_series_filled.index or pd.isna(roe_series_filled.get(y))) and has_np and has_eq and equity_series[y] != 0:
        ...
        try:
            ...
            df_quarter_table['Quý'] = df_quarter_table['_p'].apply(lambda c: f"Q{str(c).split('-Q')[1]}/{str(c).split('-Q')[0]}")
        ...
        capex_series = normalize_to_billion_vnd(find_row_series(
            ...
            ['tiền chi để mua sắm', 'purchase of fixed assets', 'capital expenditure', 'mua sắm tài sản cố định',
             'mua sắm xây dựng tài sản cố định', 'tiền chi mua sắm tscđ', 'tiền chi ra để mua sắm, xây dựng tài sản cố định',
             'purchase of property, plant and equipment', 'purchases of fixed assets']))
        ...
        if latest_fcff and latest_fcff > 0 and issue_share > 0:
            dcf_results = dcf_fcff_scenarios(
                latest_fcff=latest_fcff, shares_outstanding=issue_share, net_debt=net_debt_latest * 1e9, ticker=ticker)
            reverse_g = reverse_dcf_implied_growth(
                current_price=current_price, shares_outstanding=issue_share, latest_fcff=latest_fcff, 
                wacc=estimate_wacc(ticker), net_debt=net_debt_latest * 1e9)
        ...
        eps_adj, bvps_adj = normalize_eps_bvps_series(eps_series_filled, bvps_series_filled, outstanding_shares_series)
        ...
        valuation_methods = nine_methods_valuation(
            eps_latest=eps_latest, bvps_latest=bvps_latest, pe_series=pe_series, pb_series=pb_series,
            current_price=current_price, dcf_results=dcf_results, graham_value=graham_value, ddm_value=ddm_value,
            eps_adj=eps_adj, bvps_adj=bvps_adj, shares_series=outstanding_shares_series,
            net_profit_series=net_profit_series, dps_series=dps_series, ticker=ticker)
        ...
        valuation_package = {
            ...
            "price_series": (
                df_price.set_index('time')['close_vnd'].resample('YE').last().rename(lambda x: x.year)
        ...
        if not news_list:
            news_list = [{"title": "Không có sự kiện bất thường trong 30 ngày.", "source": "Hệ thống tự động", "url": "#", "pub_date": "—"}]

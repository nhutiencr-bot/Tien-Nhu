...
def normalize_to_billion_vnd(series):
    ...
    Dùng median-based detection thay vì ngưỡng cứng 1e11:
      - Ngưỡng cũ (> 1e11) bỏ sót LNST ngân hàng ~46 tỷ = 4.6e10 < 1e11
        → giữ nguyên đơn vị đồng → ROE/ROS tính ra hàng triệu %
      - Logic mới: nếu median tuyệt đối > 1e7 (10 triệu đồng) thì
        series đang ở đơn vị đồng → chia 1e9 về tỷ
      - Nếu median trong [100, 1e7] → đã là tỷ, giữ nguyên
      - Nếu median < 100 → có thể là tỷ nhỏ hoặc ratio, giữ nguyên
    """
    ...
    should_divide = median_abs > 1e7  # > 10 triệu → đơn vị đồng
    def _to_ty(val):
        try:
            if pd.isna(val):
                return None
    ...
    try:
        ...
        def _filter_years(s):
            if s is None or s.empty:
                return s
            # Chuẩn hóa index về int — API trả về string '2021', float 2021.0,
            # hoặc int 2021 tuỳ nguồn → isin(allowed_years) với set int sẽ miss
            # nếu index là string/float → đây là root cause mất 2021 toàn bộ mã
            try:
                s = s.copy()

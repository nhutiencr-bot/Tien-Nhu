...
def execute_equity_research_pipeline(ticker):
    try:
        ...
        # vnstock (VCI/KBS/DNSE) đôi khi chỉ trả về ít kỳ hơn số limit truyền
        # vào. 2 khối fallback CafeF phía trên chỉ kích hoạt khi series RỖNG
        # HOÀN TOÀN, không xử lý trường hợp series có dữ liệu nhưng THIẾU đúng
        # 1 năm cụ thể — đây là khối xử lý đúng trường hợp đó, dùng
        # fetch_cafef_yearly_full() cào đủ cả 4 chỉ tiêu cho riêng (các) năm
        # bị thiếu.

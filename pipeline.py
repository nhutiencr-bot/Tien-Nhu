...
BANK_TICKERS = {'VCB', 'BID', 'CTG', 'TCB', 'MBB', 'ACB', 'STB'}
OIL_TICKERS  = {'BSR', 'OIL', 'PLX', 'PVD', 'PVS', 'GAS'}
...
def normalize_net_profit_with_anchor(net_profit_raw, equity_series, roe_series):
    ...
    for year, raw_val in base.items():
        ...
        if expected == 0 or raw_val == 0 or raw_val / expected <= 0:
            fixed[year] = raw_val
            continue
        power = round(np.log10(abs(raw_val / expected)))
...
# ══════════════════════════════════════════════════════════════════════
# SOURCE SELECTION  —  probe SONG SONG thay vì tuần tự  (~2s thay vì ~8s)
# ══════════════════════════════════════════════════════════════════════
def _probe_source(ticker: str, source: str):
    """Probe 1 nguồn trong thread riêng."""
    try:
        test_end   = datetime.today().strftime('%Y-%m-%d')
        test_start = (datetime.today() - timedelta(days=5)).strftime('%Y-%m-%d')
        q = Quote(symbol=ticker, source=source)
        df = q.history(start=test_start, end=test_end, interval='1D')
...
def _pick_source_fast(ticker: str) -> str:
    """Probe tất cả nguồn SONG SONG, trả về nguồn đầu tiên thành công."""
    with ThreadPoolExecutor(max_workers=len(SOURCE_FALLBACK_ORDER)) as ex:
        futures = {ex.submit(_probe_source, ticker, s): s
                   for s in SOURCE_FALLBACK_ORDER}
        done_source = None
        for future in as_completed(futures):
            result = future.result()
            if result and done_source is None:
                done_source = result
                # Không break — để các future khác tự kết thúc (tránh thread leak)
    if done_source:
        return done_source
    raise ConnectionError(
        f"Không lấy được dữ liệu cho {ticker} từ bất kỳ nguồn nào "
        f"({', '.join(SOURCE_FALLBACK_ORDER)})."
    )


def _build_engines(ticker: str, source: str):
    """Tạo engine không probe — tức thì."""
    q = Quote(symbol=ticker, source=source)
...
def _safe_fetch(fn, default=None):
    """Gọi 1 API call, trả về default nếu lỗi."""
    try:
        result = fn()
        return result if result is not None else (
            default if default is not None else pd.DataFrame())
    except Exception:
        return default if default is not None else pd.DataFrame()


def _fetch_balance_sheet(ticker: str, period: str = 'year'):
    """Thử lần lượt các nguồn để lấy balance sheet."""
    for src in SOURCE_FALLBACK_ORDER:
        try:
            f_bs = Finance(symbol=ticker, source=src, period=period)
...
def execute_equity_research_pipeline(ticker: str):
    try:
        # ── 1. Chọn nguồn SONG SONG (~2s thay vì ~8s tuần tự) ────────
        source_used = _pick_source_fast(ticker)
        q_engine, f_engine, c_engine = _build_engines(ticker, source_used)

        if source_used != 'VCI':
            st.info(f"ℹ️ Nguồn VCI không khả dụng cho {ticker}, đang dùng: {source_used}")

        # Chỉ lấy 1 năm lịch sử giá  (đủ MA20, chart) — giảm ~2.5s
        end_date   = datetime.today().strftime('%Y-%m-%d')
        start_date = (datetime.today() - timedelta(days=365)).strftime('%Y-%m-%d')

        # ── 2. Tất cả API calls SONG SONG ────────────────────────────
        tasks = {
            "price":     lambda: q_engine.history(
        ...
        # Fallback equity từ assets – liabilities
        if equity_series.empty and not total_assets_series.empty:
            ...
            if not total_liab_series.empty:
                common = total_assets_series.index.intersection(total_liab_series.index)
                if len(common) > 0:
                    equity_series = (total_assets_series.loc[common]
                                     - total_liab_series.loc[common])

        # Fallback CafeF
        if equity_series.empty or total_assets_series.empty:
            cafef_data = fetch_cafef_balance_sheet_5y(
                ticker, end_year=datetime.today().year)
        ...
        if issue_share == 0.0 and not df_overview.empty \
                and 'charter_capital' in df_overview.columns:
            try:
                issue_share = float(df_overview['charter_capital'].iloc[0]) / 10_000
            except Exception:
                pass
        ...
        df_5y_table['ROE (%)'] = df_5y_table['Năm'].map(
            lambda y: roe_series.get(y, None))
        df_5y_table['ROA (%)'] = df_5y_table['Năm'].map(
            lambda y: roa_series.get(y, None))
        ...
        try:
            ...
            np_q    = normalize_net_profit_with_anchor(
                          fin_q['net_profit'], eq_q, fin_q['roe'])
            eps_q   = fin_q['eps']
            bvps_q  = fin_q['bvps']
            ...
            quarters = sorted(
                set(rev_q.index) | set(np_q.index) |
                set(eq_q.index)  | set(ta_q.index),
                key=lambda c: (int(str(c).split('-Q')[0]),
                               int(str(c).split('-Q')[1])))
            ...
            df_quarter_table['ROE (%)'] = df_quarter_table['_p'].map(
                lambda y: roe_q.get(y, None))
            df_quarter_table['ROA (%)'] = df_quarter_table['_p'].map(
                lambda y: roa_q.get(y, None))
        ...
        latest_fcff = None
        if cfo_series is not None and not cfo_series.empty:
            cfo_l   = get_latest(cfo_series, default=None)
            capex_l = (get_latest(capex_series, default=0.0)
        ...
        vol_vs_avg   = ((latest_vol / avg_vol_20d - 1) * 100) if avg_vol_20d > 0 else 0.0
        ...
        technical_summary = {
            ...
            "ma20":              ma20_val,
            "oil_correlation":   0.74 if ticker in OIL_TICKERS else 0.0,

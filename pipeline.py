...
def execute_equity_research_pipeline(ticker):
    try:
        ...
        # Chỉ hiển thị 5 năm gần nhất (TARGET_YEARS, đã định nghĩa ở phần
        # audit split phía trên) — một số series (đặc biệt Vốn CSH/Tổng tài
        # sản từ bảng CĐKT) có thể có lịch sử xa hơn (VD 2018-2020) do
        # nguồn dữ liệu trả về nhiều kỳ hơn các dòng khác; nếu lấy union
        # toàn bộ, bảng sẽ kéo dài về các năm cũ không đồng bộ dữ liệu
        # (nhiều ô trống). Giới hạn về đúng cửa sổ 5 năm chuẩn.
        ...
        # ── Fallback CafeF cho CFO/CapEx (khắc phục "DCF: thiếu dữ liệu dòng
        # tiền" khi vnstock không trả về báo cáo lưu chuyển tiền tệ đầy đủ cho
        # mã/kỳ này — trước đây DCF không có bất kỳ nguồn dự phòng nào cho 2
        # chỉ số này, khác với Doanh thu/LNST/Vốn CSH/Tổng tài sản đã có).
        needs_cafef_cfo = cfo_series.empty or bool(TARGET_YEARS - set(cfo_series.index))
        if needs_cafef_cfo:
            cafef_cf = fetch_cafef_yearly_full(ticker, years=sorted(TARGET_YEARS))
            cfo_series = _gapfill_series(cfo_series, cafef_cf['cfo'], "Lưu chuyển tiền HĐKD (CFO)")
            capex_series = _gapfill_series(capex_series, cafef_cf['capex'], "CapEx (mua sắm TSCĐ)")
        ...
        if latest_fcff and latest_fcff > 0 and issue_share > 0:
            # ⚠️ TRƯỚC ĐÂY: net_debt=0.0 HARDCODE — bỏ qua hoàn toàn nợ vay
            # ròng khi tính DCF, trong khi net_debt_latest ĐÃ được tính sẵn
            # ở trên (dùng cho EV/EBITDA) nhưng không được tái sử dụng ở đây.
            # Sai lệch công thức chuẩn của tác giả: equityValue = enterpriseValue
            # - netDebt. Với công ty có đòn bẩy cao, bỏ qua nợ ròng làm DCF
            # định giá equity CAO HƠN thực tế đáng kể (toàn bộ EV bị tính
            # nhầm thành equity value).
            net_debt_for_dcf = (net_debt_latest * 1e9) if net_debt_latest else 0.0
            dcf_results = dcf_fcff_scenarios(
                latest_fcff=latest_fcff, shares_outstanding=issue_share, net_debt=net_debt_for_dcf,
                scenarios=dcf_scenarios)
            reverse_g = reverse_dcf_implied_growth(
                current_price=current_price, shares_outstanding=issue_share,
                latest_fcff=latest_fcff, wacc=wacc_base, net_debt=net_debt_for_dcf)

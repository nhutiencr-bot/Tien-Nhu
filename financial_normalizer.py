...
# ─────────────────────────────────────────────────────────────────────────────
# BUG FIX: hàm _parse_year_col
#
# Vấn đề gốc: vnstock đã đổi format cột annual từ '2025' → '2025-Q4'.
# Regex cũ r'\d{4}' chỉ nhận dạng dạng '2025', bỏ sót '2025-Q4' hoàn toàn
# → toàn bộ dữ liệu 2025 bị drop ngay tầng đầu tiên.
#
# Fix: nhận diện cả hai format, parse năm bằng str(col)[:4], trả về int năm.
# ─────────────────────────────────────────────────────────────────────────────
def _parse_year_col(c):
    """
    Nhận diện cột năm theo ĐÚNG 2 format vnstock có thể trả về:
      - '2025'      (format cũ)
      - '2025-Q4'   (format mới từ ~2025, annual được đánh nhãn Q4)
    Trả về int năm nếu khớp, None nếu không phải cột năm.
    """
    c_str = str(c).strip()
    if re.fullmatch(r'\d{4}', c_str):
        return int(c_str)
    if re.fullmatch(r'\d{4}-Q4', c_str):          # annual disguised as Q4
        return int(c_str[:4])
    return None
...
def _get_year_columns(df: pd.DataFrame):
    """
    Trả về list các cột năm (cả '2025' lẫn '2025-Q4'),
    đã sắp xếp theo năm tăng dần.
    """
    ...
    for c in df.columns:
        if c in meta_cols:
            continue
        if _parse_year_col(c) is not None:
            year_cols.append(c)
    # Sắp xếp theo năm thực (int) để 2025-Q4 không bị đặt sai thứ tự
    year_cols = sorted(year_cols, key=lambda x: _parse_year_col(x))
    return year_cols
...
def _get_quarter_columns(df: pd.DataFrame):
    """
    Nhận diện cột theo quý: 'YYYY-Q1' .. 'YYYY-Q3'.
    LƯU Ý: 'YYYY-Q4' annual bị loại khỏi đây vì đã được xử lý bởi _get_year_columns.
    Tuy nhiên nếu vnstock trả Q4 thật (quarterly endpoint), nó vẫn được chấp nhận —
    phân biệt bằng context (period='quarter' vs 'year').
    """
    meta_cols = {'item', 'item_en', 'item_id'}
    q_cols = []
    for c in df.columns:
...
def find_row_series(df: pd.DataFrame, keywords, exclude_keywords=None,
    ...
    """
    period='year'    → tìm cột dạng 'YYYY' hoặc 'YYYY-Q4', key trả về là int năm.
    period='quarter' → tìm cột dạng 'YYYY-Qn', key trả về là str 'YYYY-Qn'.
    """
    ...
    for yc in year_cols:
        ...
        if pd.notna(val):
            if period == 'quarter':
                ...
                # KEY luôn là int năm, bất kể cột là '2025' hay '2025-Q4'
                result[_parse_year_col(yc)] = float(val)
